...
def test_ensure_npc_profile_with_npc_profile_block(db):
    ...
    assert created_npc.trust == 0.5  # Trust from npc_profile default_trust
...
def test_ensure_npc_profile_with_npcs_present_fallback(db):
    ...
    created_npc = get_npc_by_id(player_id, npc_id, db)
    assert created_npc is not None
    assert created_npc.name == "Companion"  # Default name from companion terminology
    assert created_npc.trust == 0.3  # Default trust value
...
def test_ensure_npc_profile_mixed_sources(db):
    ...
    assert npc_2 is not None
    assert npc_2.name == "Companion"  # Default name from companion terminology
...
def test_ensure_npc_profile_invalid_data(db):
    ...
    scene = {
        ...
        "npcs_present": ["invalid-uuid-format", 123, None]
    }
