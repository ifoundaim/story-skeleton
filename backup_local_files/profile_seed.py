...
def ensure_npc_profile(scene: dict, player_id: str) -> None:
    ...
    try:
        def _add_stub(npc_id: str, name: str = None, recruitable: bool = True,
                      default_trust: float = 0.3, archetype: str = None) -> None:
            # Check if NPC already exists
            existing_npc = get_npc_by_id(player_id, npc_id, db)
            if existing_npc is None:
                # Create new NPC with provided or default values
                npc_name = name or "Companion"  # Use "Companion" terminology
                create_default_npc(player_id, npc_id, npc_name, db, initial_trust=default_trust, archetype=archetype)
                print(f"Created companion profile for {npc_id} ({npc_name})")
        ...
        for prof in profiles:
            if isinstance(prof, dict) and "id" in prof:
                try:
                    ...
                    print(f"Skipping invalid UUID format in npc_profile: {prof['id']}")
                    continue
        ...
        for npc_id in scene.get("npcs_present", []):
            if isinstance(npc_id, str):
                try:
                    ...
                    print(f"Skipping invalid UUID format: {npc_id}")
                    continue
